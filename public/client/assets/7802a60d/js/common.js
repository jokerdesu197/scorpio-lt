...
function sticky() {
    ...
    if (window_width <= 991) {
        $("#col_right").find(".sticky").removeAttr('style');
        return false;
    }
    var position_start = $("#col_left").offset().top;
    $(window).scroll(function () {
        var left_height = $("#col_left").outerHeight();
        var right_height = $("#col_right").find(".sticky").outerHeight();
        ...
        if (y < position_start) {
            ...
            console.log(y + ' - ' + bottom + ' - ' + offset)
            if (offset < bottom) {
                $("#col_right").find(".sticky").css({ "margin-top": offset });
            }
            else {
                $("#col_right").find(".sticky").css({ "margin-top": bottom });
            }
        }
    });
}
